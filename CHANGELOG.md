...
- **Android Gateway / Web** — Istemci `RD Gateway (443)` `.rdp` ve `Web (tarayici)` HTML5 yolunu listeler. WAN icin Gateway onerilir; sertifika `/gateway.cer`. GHA APK debug anahtariyla imzalanir (imzasiz APK Android'de yuklenmez). Android `.rdp` RemoteApp (`remoteapplicationmode:i:1` + EXE kabugu); tam masaustu oturumu acilmaz.
